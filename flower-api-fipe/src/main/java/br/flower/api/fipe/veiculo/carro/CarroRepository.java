...
@Repository
public interface CarroRepository extends JpaRepository<Carro, Veiculo.VeiculoPk> {

	
	@Query("SELECT DISTINCT u.marca from Carro u")
	List<Marca> findDistinctMarcaId();
	
	List<Modelo> findDistinctByMarcaMarcaId(Long marca);
	
	@Query("SELECT DISTINCT u.modelo from Carro u WHERE u.marca.id=?1")
	List<Modelo> findDistinctModelo(Long marcaId);
	
	@Query("SELECT DISTINCT u.anoVeiculo from Carro u WHERE u.marca.id=?1 AND u.modelo.id=?2")
